...
import { MOCK_USERS, MOCK_ABSENCE_TYPES } from "@/lib/mock-data";
...
export default function ScheduleCalendar() {
  ...
  const directReports = MOCK_USERS.filter(u => u.turma === MOCK_USERS.find(user => user.id.toString() === selectedUser)?.turma && u.id.toString() !== selectedUser);
        ...
        <div className="space-y-6">
                      ...
                      >
                        ...
                        <div className="flex-1 space-y-2 overflow-y-auto pr-1 custom-scrollbar">
                          {dayAbsences.map(absence => {
                            ...
                            const absenceType = MOCK_ABSENCE_TYPES.find(t => t.type === absence.type) || MOCK_ABSENCE_TYPES.find(t => t.type === "outro") || MOCK_ABSENCE_TYPES[0];
                            const colorClass = getAbsenceColorClass(absenceType.color);
                ...
                <div className="flex flex-wrap gap-4">
                  <div className="flex items-center gap-3 bg-primary/10 px-4 py-2 rounded-full border border-primary/20">
                    <Avatar className="h-10 w-10 border-2 border-background shadow-sm">
                      <AvatarImage src={MOCK_USERS.find(u => u.id.toString() === selectedUser)?.avatar} />
                      <AvatarFallback>VO</AvatarFallback>
                    </Avatar>
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-primary">Você</span>
                      <span className="text-xs text-muted-foreground">{MOCK_USERS.find(u => u.id.toString() === selectedUser)?.name}</span>
                    </div>
                  </div>
          ...
          <div className="flex flex-wrap items-center gap-3 p-3 bg-muted/30 rounded-lg border mt-6">
            ...
            {MOCK_ABSENCE_TYPES.map(type => (
              <div key={type.id} className={cn("px-2.5 py-1 rounded text-xs font-medium border shadow-sm", getAbsenceColorClass(type.color))}>
                {type.name}
              </div>
            ))}
