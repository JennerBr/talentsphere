...
export function OrgChart({ data }: { data: OrgPosition[] }) {
  ...
  const buildTree = (flatData: OrgPosition[]): OrgPosition[] => {
    ...
    flatData.forEach(item => {
      dataMap.set(item.id, { ...item, allocated: item.allocated || 0, children: [] });
    });
    ...
    // Third pass: calculate totals bottom-up (post-order traversal)
  ...
  const handleSave = () => {
    // Implementar lógica real de salvar aqui futuramente
    setEditingPosition(null);
  };

  const handleAdd = () => {
    // Implementar lógica real de adicionar aqui futuramente
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  type="number"
                  min="1"
                  defaultValue={editingPosition.chairs}
                />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="parent_id">Posição Pai (Reporta a)</Label>
                <Select defaultValue={editingPosition.parent_id?.toString() || "none"}>
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    {positions
                      .filter(p => p.id !== editingPosition.id) // Não pode reportar a si mesmo
                      .map(p => (
      ...
      <Dialog open={!!addingToPosition} onOpenChange={(open) => !open && setAddingToPosition(null)}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdd} disabled={!newRole}>Adicionar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!viewingPosition} onOpenChange={(open) => !open && setViewingPosition(null)}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              <span className="font-semibold text-foreground">{viewingPosition?.allocated || 0}</span> pessoa(s) alocada(s) de <span className="font-semibold text-foreground">{viewingPosition?.chairs || 0}</span> cadeira(s) diretas previstas
            </p>
          </DialogHeader>
          <div className="flex flex-col gap-3 py-4 max-h-[60vh] overflow-y-auto pr-2">
            {viewingPosition && (viewingPosition.allocated || 0) > 0 ? (
              // Use a mix of users to mock the occupants based on allocation count
              Array.from({ length: viewingPosition.allocated || 0 }).map((_, i) => {
                // Loop through MOCK_USERS if there are more allocated than available mock users
                  ...
                  <div key={i} className="flex items-center justify-between p-3 border rounded-lg bg-card hover:bg-muted/50 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">{user.role} {user.seniority}</p>
                      </div>
                    </div>
          ...
          <DialogFooter className="sm:justify-between flex-row-reverse sm:flex-row">
            ...
            <Button onClick={() => {
               // Ação mockada para adicionar pessoa
               alert("Modal de busca de colaboradores seria aberto aqui.");
            }} className="gap-2">
              <Plus className="h-4 w-4" />
              Alocar Pessoa
            </Button>
