...
import { useMembers, useLogTypes, useCurrentMember } from "@/lib/use-data";
...
export default function MemberLogbook() {
  const { data: employees = [] } = useMembers();
  const { data: logTypes = [] } = useLogTypes();
  const { data: currentMember } = useCurrentMember();
  const currentUser = currentMember ?? employees[0];
  ...
  const selectedMate = employees.find(u => u.id.toString() === selectedMateId);

  const getRelationship = (mateId: string) => {
    if (!mateId || !currentUser) return "";
    const mate = employees.find(u => u.id.toString() === mateId);
    if (!mate) return "";
    
    if (mate.id === currentUser.id) return "Você";
                    ...
                    <div className="flex items-center justify-between p-3 border rounded-lg bg-card">
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 border">
                          <AvatarImage src={selectedMate.avatar ?? undefined} />
