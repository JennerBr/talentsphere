...
import { useMembers, useSkillCategories, useMemberSkills, useAddMemberSkill, useCurrentMember } from "@/lib/use-data";
...
export default function MemberPDI() {
  const { data: currentMember, isLoading: isAuthLoading, isError: isAuthError } = useCurrentMember();
  ...
  const { data: skillCategoryList = [] } = useSkillCategories();
  const { data: userSkills = [] } = useMemberSkills(selectedUserId);
  const { data: mentorSkills = [] } = useMemberSkills(selectedMentorId ?? 0);
  const addMemberSkill = useAddMemberSkill(selectedUserId);
  const { toast } = useToast();

  useEffect(() => {
    if (currentMember?.id && selectedUserId === 0) {
      setSelectedUserId(currentMember.id);
    }
  }, [currentMember?.id, selectedUserId]);

  if (isAuthLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="text-muted-foreground">Carregando PDI...</div>
      </div>
    );
  }

  if (isAuthError || !currentMember) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
  ...
  const currentMentor = employees.find(u => u.id === selectedMentorId);

  const allowedUsersToSelect = employees.filter(u => !currentUser || u.turma === currentUser.turma || u.id === currentUser.id);
                  ...
                  <Dialog open={isMentorModalOpen} onOpenChange={setIsMentorModalOpen}>
                    ...
                    <DialogContent>
                      ...
                      <div className="grid grid-cols-1 gap-2 py-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        {employees.filter(u => u.id !== currentUser?.id).map(u => (
                          <div 
                            key={u.id} 
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                      <BarChart data={categorySkills} layout="vertical" margin={{ top: 20, right: 20, left: 20, bottom: 25 }}>
                                        ...
                                        <XAxis 
                                          ...
                                          axisLine={false} 
                                          tickLine={false} 
                                          tickCount={(category.niveis ?? 3) + 1} 
                                          domain={[0, category.niveis ?? 3]} 
                                        />
                                        <YAxis 
                                          dataKey="name" 
                                          type="category"
                  ...
                  <Dialog open={isAddSkillModalOpen} onOpenChange={setIsAddSkillModalOpen}>
                    ...
                    <DialogContent>
                      ...
                      <DialogFooter>
                        ...
                        <Button
                          disabled={!newSkillName.trim() || addMemberSkill.isPending}
                          onClick={() => {
                            addMemberSkill.mutate(
                              {
                                name: newSkillName.trim(),
                                type: newSkillType,
                                level: "Básico",
                                desired: newSkillDesired,
                              },
                              {
                                onSuccess: () => {
                                  setIsAddSkillModalOpen(false);
                                  setNewSkillName("");
                                  toast({ title: "Habilidade adicionada ao PDI." });
                                },
                                onError: (err) => {
                                  toast({ title: "Erro ao adicionar habilidade", description: err.message, variant: "destructive" });
                                },
                              }
                            );
                          }}
                        >
                          {addMemberSkill.isPending ? "Adicionando..." : "Adicionar"}
                        </Button>
                      </DialogFooter>
                    </DialogContent>
                  </Dialog>
